...
def _train_or_test(model, dataloader, optimizer=None, class_specific=True, use_l1_mask=True,
    ...
    for i, (image, label, patient_id) in enumerate(dataloader):
        ...
        fine_annotation = fine_annotation.to(mps_device)
        input = image.to(mps_device)
        target = label.to(mps_device)
        ...
        with grad_req:
            ...
            if class_specific:
                ...
                prototypes_of_correct_class = torch.t(model.module.prototype_class_identity[:,label]).to(mps_device)
                ...
                if use_l1_mask:
                    l1_mask = 1 - torch.t(model.module.prototype_class_identity).to(mps_device)
